...
const COLOR_TASK     := Color(0.67, 1, 0.60)
const COLOR_ERROR    := Color(1, 0.41, 0.41)
const COLOR_WARNING  := Color(1, 0.78, 0.29)
const COLOR_INFO     := Color(0.51, 0.85, 1)
const COLOR_REMINDER := Color(1, 0.53, 0.90)
...
# if not 'silent', then it prints out the 'task' or 'error' messages
# provided in the 'msgs' Dictionary, and reports the benchmark time
func run_task(msgs:Dictionary[String, String], silent:bool, f:Callable) -> Variant:
	var has_messages := not msgs.is_empty()
	if silent or not has_messages: return f.call()
